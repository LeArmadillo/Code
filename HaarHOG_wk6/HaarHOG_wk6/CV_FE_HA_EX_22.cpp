  ...
  fflush(stdout);
	...
	for( i = 0; i < found_filtered.size(); i++ )
	{
		...
		equalizeHist( frame_gray, frame_gray );
		  //-- Detect faces
		  one_cascade.detectMultiScale( frame_gray, one, 1.1, 2, 0|CV_HAAR_SCALE_IMAGE, Size(10, 10) );
		  two_cascade.detectMultiScale( frame_gray, two, 1.1, 2, 0|CV_HAAR_SCALE_IMAGE, Size(10, 10) );
		  ...
		  for( int i = 0; i < one.size(); i++ )
		  {
			...
			imshow( temp_window_name3, regionOfInterest );
		  }
		  ...
		  for( int i = 0; i < two.size(); i++ )
		  {
			...
			imshow( temp_window_name4, regionOfInterest );
		  }
