...
#include <ctype.h>
 ...
 void IB_trackObject( Point location, Mat &frame, vector<Point> &points, int fc );
     ...
     while( true )
     {
		if( !( frameCount == 3 ) || modelBuildCount > 0 )	{
			...
			if( totalFrames < 0 )	{
				...
				//trackFrame = Mat( wholeImage.rows, wholeImage.cols, CV_8UC3 );
				//trackFrame.setTo( Scalar() );
				//trackingArray = new Point[totalFrames];
			}
			*/
		} else {
			modelBuildCount++;
		...
		if( IB_colourClassification( loc1, con1, bsKMeansROI, location ) )	{
		...
		IB_trackObject( trackingLocation, wholeImage, trackingArray, frameCount );
...
void IB_BS( Mat BS_in, Mat &bsMask, Mat &BS_out )
{
	...
	//fillHorrizontalRegions( BS_mask, c0 );
	//imshow( "C", BS_mask );
	//moveWindow( "A", 1300, 0 );
	//moveWindow( "B", 1300, 300 );
	//moveWindow( "C", 1300, 700 );
	//moveWindow( "D", 700, 700 );
...
bool IB_colourClassification( Rect &loc1, float &con1, Mat bsKMeansROI, Rect &location )	
{
	...
	//if( 
		helper_colourClassification( "B", B, symetric, contoursB, true, location );// &&
...
bool helper_colourClassification( string name, Mat chan, bool *symetric, vector<vector<Point>> contours, bool h, Rect &location ) 
{
	copyMakeBorder( chan, chan, 20, 20, 20, 20, BORDER_CONSTANT );
	...
	for( size_t i=0; i<contours.size(); i++ )	{
		...
		if( rBounds[i].height/rBounds[i].width > 2 || rBounds[i].height/rBounds[i].width < 1/2 )	{
