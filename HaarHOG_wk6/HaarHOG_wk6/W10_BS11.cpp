...
bool helper_colourClassification( string name, Mat chan, bool *symetric, vector<vector<Point>> contours, bool h, Rect &location ) 
{
	...
	for( size_t i=0; i<contours.size(); i++ )	{
		...
		if( cBounds[i].size.height/cBounds[i].size.width > 3 || cBounds[i].size.height/cBounds[i].size.width < 1/3 )	{
...
void IB_trackObject( Rect location, Mat &frame )	{

	int posX = location.x + location.width/2;
    int posY = location.y + location.height/2;        
        
    if(lastX>=0 && lastY>=0 && posX>=0 && posY>=0)
    {
		// Draw a yellow line from the previous point to the current point
		line( frame, Point(posX, posY), Point(lastX, lastY), cvScalar(0,255,255), 4 );
    }
	lastX = posX;
	lastY = posY;
}