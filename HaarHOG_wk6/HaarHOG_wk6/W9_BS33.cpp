 ...
 int main( int argc, const char** argv )
 {
   ...
   if( capture )
   {
     while( true )
     {
		...
		moveWindow( "Background Subtraction", 650, 0 );
		...
		moveWindow( "Region of Interest", 0, 400 );
		imshow( "B.S. R.O.I.", bsROI2 );
		moveWindow( "B.S. R.O.I.", 410, 400 );
		imshow( "KMeans ROI", bsKMeansROI2 );
		moveWindow( "KMeans ROI", 820, 400 );
...
void IB_BS( Mat BS_in, Mat &bsMask, Mat &BS_out )
{
	...
	hullPlotter( BS_C_mask, c0, "HULL FOUR", 0, -800 );
	hullPlotter( BS_C_mask, a0, "HULL FOUR", 0, -800 );
