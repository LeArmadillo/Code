...
struct prResults
{
 ...
 void IB_edgeAnalysis( Mat frameE, Mat bsEMask, Mat &orientationSobel, Mat &edgeCanny, Mat &edgeSobel );
 ...
 bool edgeCountSansOrientation( Mat &frameOS, Mat &frameEC, cv::Rect &location );
 ...
 Mat get_hogdescriptor_visu( Mat& origImg, vector<float>& descriptorValues, int x_IN, int y_IN );
...
void IB_Run_All_Test_Data( ofstream &file )
{
	...
	*/
	sprintf( vidname, "C:/Users/Ian Beatty-Orr/Documents/Y5 Eng/5E1/Recordings/M4H00740.MP4.AVI" );
	sprintf( annotatename, "C:/Users/Ian Beatty-Orr/Documents/Y5 Eng/5E1/Recordings/Annotations/IB_A_740" );
	//cout << endl << "CALLING NEXT VIDIEO: " << num << endl << vidname << endl << annotatename << endl << endl;
	IB_Backpack_Detection( vidname, annotatename, result );
...
void IB_Backpack_Detection( char vidname[100], char annotatename[100], prResults &results )
 {
	...
	vidLenght = cvGetCaptureProperty( capture, CV_CAP_PROP_FRAME_COUNT );
   ...
   if( capture )
   {
     while( true )
     {
		...
		if( true )	{
			...
			IB_edgeAnalysis( wholeROI, bsMaskROI, orientationSobel, edgeCanny, edgeSobel );	
			if( false && edgeCount( orientationSobel, location ) )	{
			...
			if( true && edgeCountSansOrientation( orientationSobel, edgeSobel, location ) )	{	
				rectangle( wholeImage, Point( ROI_TL+location.tl() ), Point( ROI_TL+location.br() ), Scalar( 255, 0, 0 ), 3 );
				Point trackingLocation = Point( ROI_TL.x + location.x + (location.width/2), ROI_TL.y + location.y + (location.height/2) );
				IB_trackObject( true, trackingLocation, wholeImage, trackingArray, frameCount );
				detectedLocation = Rect( Point( ROI_TL+location.tl() ), Point( ROI_TL+location.br() ) );
...
void IB_detectAndDisplay( bool active, Mat wholeImage, Mat bsImage, Mat bsMask, Mat &wholeROI, Mat &bsROI, Mat &bsMaskROI, Point &ROI_TL )
{
  ...
  HOGDescriptor hog, hog2;
  hog.setSVMDetector(HOGDescriptor::getDefaultPeopleDetector());

	vector<float> descriptorsValues;
	vector<Point> locations;
	/*
	hog.compute( wholeImage, descriptorsValues, Size(0,0), Size(0,0), locations);
	Mat hiol = get_hogdescriptor_visu( wholeImage, descriptorsValues, 640, 360 );
	imshow( "YOWZA", hiol );
	*/
  ...
  fflush(stdout);
	...
	for( i = 0; i < found_filtered.size(); i++ )
	{
		...
		ROI_TL = Point( x1, y1 );
		//Mat testROI = wholeImageT( Rect( wholeImage.cols/4, wholeImage.rows/4, 128, 128 ) );
		//cout << "YOWZA234567890   " << testROI.cols << "  " << testROI.rows << endl;
		/*
		Mat testROI; 
		float s = 200/wholeROI.rows;
		resize( wholeROI, testROI, Size(wholeROI.cols*s,wholeROI.rows*s) );
		hog2.compute( testROI, descriptorsValues, Size(0,0), Size(0,0), locations);
		Mat hiol = get_hogdescriptor_visu( testROI, descriptorsValues, testROI.cols, testROI.rows );
		imshow( "YOWZA", hiol );
		resize( wholeROI, wholeROI, Size(wholeROI.cols*5,wholeROI.rows*5) );
		imshow( "YOWZB", wholeROI );
		*/
...
void IB_edgeAnalysis( Mat frameE, Mat bsEMask, Mat &orientationSobel, Mat &edgeCanny, Mat &edgeSobel )
{
	...
	*/
	...
	for( int row=0; row<t0.rows; row++ )	{
		for( int col=0; col<t0.cols; col++ )	{
			if( t1.at<uchar>(row,col) == 0 )	{
				//t0.at<uchar>(row,col) = 128;
				t12.at<uchar>(row,col) = 255;
				//t12.at<uchar>(row,col) = 0;
			} else if( t1.at<uchar>(row,col) == 128 ) {
				//t0.at<uchar>(row,col) = 0;
				t12.at<uchar>(row,col) = 0;
				//t12.at<uchar>(row,col) = 255;
			} 
...
// Change to Contour Follower
{
	...
	vector<Vec4i> h0;
	Mat tempCannyEdges = frameEC.clone();
	resize( tempCannyEdges, tempCannyEdges, Size( tempCannyEdges.cols*5, tempCannyEdges.rows*5 ) );
	imshow( "Canny Edges", tempCannyEdges );
	moveWindow( "Canny Edges", 0, 800 );
	tempCannyEdges = frameOS.clone();
	resize( tempCannyEdges, tempCannyEdges, Size( tempCannyEdges.cols*5, tempCannyEdges.rows*5 ) );
	imshow( "Canny Orientation", tempCannyEdges );
	moveWindow( "Canny Orientation", 0, 800 );
	...
	int* contourLen = new int[l];
	bool* contourValid = new bool[l];
	for( int i=0; i<l; i++ )	{
		start[i] = c0[i].front();
		...
		contourLen[i] = sqrt( pow( deltaX, 2 ) + pow( deltaY, 2 ) );
		if( contourLen[i] < frameEC.rows/3 )	{
			contourValid[i] = false;
		} else {
			contourValid[i] = true;
		}
	...
	Mat remainingEdges = frameEC.clone();
	remainingEdges.setTo( Scalar() );
	
	for( int j=0; j<l; j++ )	{
		if( contourValid[j] )	{
			drawContours( remainingEdges, c0, j, Scalar( 255 ) );
		}
	}
	...
	for( int j=0; j<l; j++ )	{
		for( int k=0; k<l; k++ )	{
			if( contourOri[j] < contourOri[k]*1.01 && contourOri[j] > contourOri[k]*0.99 )	{
				if( ( start[j].x < start[k].x*1.01 && start[j].x > start[k].x*0.99 ) ||
					( start[j].x < end[k].x*1.01 && start[j].x > end[k].x*0.99 ) ||
					( end[j].x < start[k].x*1.01 && end[j].x > start[k].x*0.99 ) ||
					( end[j].x < end[k].x*1.01 && end[j].x > end[k].x*0.99 ) )	{
						cout << "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAA" << endl;
						vector<vector<Point>> oneContour;
						oneContour.push_back( c0[j] );
						drawContours( remainingEdges, oneContour, -1, Scalar( 255, 0, 255 ) );
						break;
	...
	*/
	...
	frameEC = remainingEdges;
	
	resize( remainingEdges, remainingEdges, Size( remainingEdges.cols*5, remainingEdges.rows*5 ) );
	cv::imshow( "Remaining Canny Edges", remainingEdges );
	cv::moveWindow( "Remaining Canny Edges", 300, 800 );
...
void IB_compareToAnnotatedData( bool active, Rect edgeLocation, int frameCount )
{
	...
	if( !backpack.rect )	{
		int f1 = (frameCount/25)*25;
		int f2 = f1 + 25;
		if( f2 > vidLenght )	{
			return;
		}
		perBackpackData F1 = PointerToPointerData[f1][0];
		perBackpackData F2 = PointerToPointerData[f2][0];
		backpack.loc.x = F1.loc.x + (F2.loc.x - F1.loc.x)*((frameCount - f1)/(f2 - f1));
		backpack.loc.y = F1.loc.y + (F2.loc.y - F1.loc.y)*((frameCount - f1)/(f2 - f1));
	}
...
void IB_HOG( Mat &wholeROI_IN, Mat &bsMaskROI_IN )
{
	...
	if( width < 11 || height < 11 )	{
		cout << "ERROR: StrapRegion is too small for custom HOG method expanding!  " << width << "  " << height <<  endl;
		width = 11;
		if( l[0] > wholeROI.cols-12 )	{
			l[0] = wholeROI.cols-12;	
		}
		if( r[0] > wholeROI.cols-12 )	{
			r[0] = wholeROI.cols-12;	
		}
	} 
	*/
	...
	vector<float> descriptorsValues;
	vector<Point> locations;
	//float s = 200/leftStrapRegion.rows;
	resize( leftStrapRegion, leftStrapRegion, Size(leftStrapRegion.cols*10,leftStrapRegion.rows*10) );
	hog.compute( leftStrapRegion, descriptorsValues, Size(0,0), Size(0,0), locations);
	Mat hiol = get_hogdescriptor_visu( leftStrapRegion, descriptorsValues, leftStrapRegion.cols, leftStrapRegion.rows );
	imshow( "Left Strap HOG", hiol );
	imshow( "Left Strap Non-Hog", leftStrapRegion );
	resize( rightStrapRegion, rightStrapRegion, Size(rightStrapRegion.cols*10,rightStrapRegion.rows*10) );
	hog.compute( rightStrapRegion, descriptorsValues, Size(0,0), Size(0,0), locations);
	hiol = get_hogdescriptor_visu( rightStrapRegion, descriptorsValues, rightStrapRegion.cols, rightStrapRegion.rows );
	imshow( "Right Strap HOG", hiol );
	imshow( "Right Strap Non-Hog", rightStrapRegion );
	...
	if( leftStrapRegion.cols < 11 || leftStrapRegion.rows < 11 )	{
		cout << "ERROR: leftStrapRegion is too small for custom HOG method!  " << leftStrapRegion.cols << "  " << leftStrapRegion.rows <<  endl;
	} else {
		IB_HOG_helper( leftStrapRegion );
	}
	if( rightStrapRegion.cols < 11 || rightStrapRegion.rows < 11 )	{
		cout << "ERROR: leftStrapRegion is too small for custom HOG method!" << rightStrapRegion.cols << "  " << rightStrapRegion.rows << endl;
	} else {
		IB_HOG_helper( rightStrapRegion );
	}
...
void IB_HOG_helper( Mat strapRegion )
{
	...
	for( int k=2; k<cellCols*3; k=k+3 )	{
		for( int l=2; l<cellRows*3; l=l+3 )	{
			...
			for( int v=-1; v<=1; v++ )	{
				for( int j=-1; j<=1; j++ )	{
					//cout << " ???? " << v << " ; " << j << endl;
					double qa = strapRegionGrayscale.at<uchar>(l+v, k+v-1);
					double qs = strapRegionGrayscale.at<uchar>(l+v, k+v+1);
					double qd = strapRegionGrayscale.at<uchar>(l+v-1, k+v);
					double qf = strapRegionGrayscale.at<uchar>(l+v+1, k+v);
					cout << qa << " " << qs << " " << qd << " " << qf << endl;
					//cout << strapRegionGrayscale.at<uchar>(l+v, k+v-1) << " " << strapRegionGrayscale.at<uchar>(l+v, k+v+1) << " " << strapRegionGrayscale.at<uchar>(l+v-1, k+v) << " " << strapRegionGrayscale.at<uchar>(l+v+1, k+v) << endl;
					double magX = strapRegionGrayscale.at<uchar>(l+v, k+v-1) - strapRegionGrayscale.at<uchar>(l+v, k+v+1);
					double magY = strapRegionGrayscale.at<uchar>(l+v-1, k+v) - strapRegionGrayscale.at<uchar>(l+v+1, k+v);
					tempMag[v+1][j+1] = sqrt( pow( 2, magX ) + pow( 2, magY ) );
					tempOri[v+1][j+1] = atan( magY/magX ) * 180/PI + 90; 
					if( tempOri[v+1][j+1] > 180 ||  tempOri[v+1][j+1] < 0 )	{
						cout << "ERROR:  angle is not within the range 0-180!   " << tempOri[v+1][j+1] << endl;
						tempOri[v+1][j+1] = 0;
					}
				}
			}
			for( int i=-1; i<=1; i++ )	{
				for( int j=-1; j<=1; j++ )	{
					int bin = tempOri[i+1][j+1]/20;
					cellOri[(k-2)/3][(l-2)/3][bin] = cellOri[(k-2)/3][(l-2)/3][bin] + tempMag[i+1][j+1];
				}
			}
		}
	}
	...
	Mat hist( 5*blockRows, 5*blockCols, CV_8UC1, Scalar( 0 ) );
	for( int i=0; i<blockCols; i++ )	{
		for( int j=0; j<blockRows; j++ )	{
			Mat histAux = hist.colRange( i*5, i*5+5 ).rowRange( j*5, j*5+5 );
			if( histAux.rows == 0 || histAux.cols == 0 )	{
				cout << "OH NOs!" << endl;
			}
			double l2norm[9] = {0};
			for( int k=0; k<2; k++ )	{
	...
	//resize( hist, hist, Size( 10*hist.cols, 10*hist.rows ) );
	imshow( "HIST", hist );
}
...
{   
    Mat color_origImg;
	cvtColor(origImg, origImg, CV_RGB2GRAY);
	//resize(origImg, origImg, Size(64,128) );
    ...
    int cells_in_x_dir = x_IN / cellSize;
    int cells_in_y_dir = y_IN / cellSize;
...
Mat get_hogdescriptor_visu_ORIGIONAL(Mat& origImg, vector<float>& descriptorValues, int x_IN, int y_IN )
{   
    Mat color_origImg;
	cvtColor(origImg, origImg, CV_RGB2GRAY);
	//resize(origImg, origImg, Size(64,128) );
